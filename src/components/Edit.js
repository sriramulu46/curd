...
class Edit extends Component
{
	...
	UNSAFE_componentWillReceiveProps(user_data) {
		this.setState({
			...
			EmpName  : user_data.user.EmpName,
    ...
    checkValidation(fields) {
		...
		if(fields.EmpName.length === 0 || fields.mobile_no.length === 0 || fields.email.length === 0) {
			return true;
		} else {
			return false;
		}
			        ...
			        <form onSubmit={this.handleUpdateUser}>
			      		<div className="modal-body">
			          		<div className="form-group">
			            		<label htmlFor="EmpName" className="col-form-label">User name:</label>
			            		<input type="text" className={`form-control form-control-sm ${this.hasErrorFor('EmpName') ? 'is-invalid' : ''}`}
			            		 id="EmpName" name="EmpName" placeholder="User name" onChange={this.handleInputFieldChange} value={this.state.EmpName}/>
